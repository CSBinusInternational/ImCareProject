...
class PDF extends TCPDF {
    ...
    function Header() {
        ...
        //Judul dalam bingkai
        $this->Cell(0, 5, 'Jadwal Mengajar', 0, 1, 'C');
...
$pdf->SetTitle('Jadwal Mengajar '.$fullname);
...
$pdf->Cell(0, 0, 'Jadwal Mengajar '.$fullname." ".$periode->getPeriodename(), 0, 1, 'C');
...
$tbl .= '<th style="width: 20%;">Hari </th>';
$tbl .= '<th style="width: 25%;">Waktu</th>';
$tbl .= '<th style="width: 25%;">Pelajaran</th>';
$tbl .= '<th style="width: 20%;">Kelas</th>';
$tbl .= '</tr>';
//Header Tabel
...
while ($iterator -> valid()) 
{
    ...
    $tbl .= "<td>" . $iterator->current()->getAwal()." - ".$iterator->current()->getAkhir() . "</td>";
    $tbl .= "<td>" . $iterator->current()->getLesson()->getLessonname() . "</td>";
    $tbl .= "<td>" . $iterator->current()->getKelas()->getClasslevel().$iterator->current()->getKelas()->getNamakelas() . "</td>";
    $tbl .= "</tr>";
...
$tbl .= '<table cellspacing="0" cellpadding="3" border="1">';
$tbl .= '<tr bgcolor="lightgreen" style="font-weight:bold; text-align:center;">';
$tbl .= '<td colspan="5"><b>Jadwal Ekstrakurikuler</b></td>';
$tbl .= '</tr>';

$tbl .= '<tr bgcolor="lightblue" style="font-weight:bold; text-align:center;">';
$tbl .= '<th style="width: 10%;">No</th>';
$tbl .= '<th style="width: 20%;">Hari </th>';
$tbl .= '<th style="width: 20%;">Waktu</th>';
$tbl .= '<th style="width: 20%;">Ekstrakurikuler</th>';
$tbl .= '<th style="width: 30%;">Deskripsi</th>';
$tbl .= '</tr>';
